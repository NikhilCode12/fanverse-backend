import TenToTwelve from "../../models/BallByBall/TenToTwelve.js";
...
export const getContestById = async (req, res) => {
  try {
    const contest = await TenToTwelve.findById(req.params.id);
    if (!contest) {
...
export const getAllContests = async (req, res) => {
  try {
    const contests = await TenToTwelve.find();
    if (!contests) {
...
export const updateContestById = async (req, res) => {
  try {
    const updatedContest = await TenToTwelve.findByIdAndUpdate(
      req.params.id,
      req.body,
      {
        new: true,
...
export const deleteContestById = async (req, res) => {
  try {
    const deleteContest = await TenToTwelve.findByIdAndDelete(req.params.id);
    if (!deleteContest) {
      return res.status(404).json({ error: "Contest not found" });
    }
...
export const decreaseSpots = async (req, res) => {
  try {
    ...
    const updatedContest = await TenToTwelve.findById(req.params.id);
