import ThirteenToFifteen from "../../models/BallByBall/ThirteenToFifteen.js";
...
export const getAllMatchContest = async (req, res) => {
  try {
       const contest = await ThirteenToFifteen.find({matchId: req.body.matchId});
    if (!contest) {
...
export const getContestById = async (req, res) => {
  try {
    const contest = await ThirteenToFifteen.findById(req.params.id);
    if (!contest) {
...
export const getAllContests = async (req, res) => {
  try {
    const contests = await ThirteenToFifteen.find();
    if (!contests) {
...
export const updateContestById = async (req, res) => {
  try {
    const updatedContest = await ThirteenToFifteen.findByIdAndUpdate(
      req.params.id,
      req.body,
      {
        new: true,
...
export const deleteContestById = async (req, res) => {
  try {
    const deleteContest = await ThirteenToFifteen.findByIdAndDelete(req.params.id);
    if (!deleteContest) {
      return res.status(404).json({ error: "Contest not found" });
    }
...
export const decreaseSpots = async (req, res) => {
  try {
    ...
    await ThirteenToFifteen.updateOne(
      { _id: req.params.id },
      { $inc: { spotsLeft: -1 } }
    );

    const updatedContest = await ThirteenToFifteen.findById(req.params.id);
